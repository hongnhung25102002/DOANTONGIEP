    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(tblKhachHang objUser)
        {
            if (ModelState.IsValid)
            {
                ...
                var objNV = db.tblNhanViens.Where(a => a.tai_khoan.Equals(objUser.ma_kh) && a.mat_khau.Equals(objUser.mat_khau)).FirstOrDefault();
                //if (obj != null)
                //{
                //    Session["NV"] = obj;
                ...
                {
                    Session["KH"] = obj;
                    Session["TenChucVu"] = null;
                    Session["NV"] = null;
                    return RedirectToAction("BookRoom", "Home");
                }else if (objNV != null)
                {
                    Session["NV"] = objNV;
                    Session["TenChucVu"] = objNV.tblChucVu.chuc_vu;
                    Session["KH"] = null;
        ...
        [HttpGet]
        public JsonResult XoaPhieuDatPhongUp(int? id)
        {
            tblKhachHang kh = new tblKhachHang();
            int status = 0;
            if (Session["KH"] != null)
